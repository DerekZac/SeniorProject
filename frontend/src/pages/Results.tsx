...
import {
  ArrowRight, Star, ArrowUpRight, Globe, FileText, Search, Twitter,
  Sparkles, TrendingUp, TrendingDown, Loader2,
} from 'lucide-react';
import { SkeletonResultPage } from '../components/Skeleton';
import { logger } from '../lib/logger';
import { api, type CoinMarketDetail, type NewsItem, type AISentiment } from '../lib/api';
...
// Keep the news list relevant to the coin without pulling in api internals.
function relevantNews(news: NewsItem[], name: string, ticker: string): NewsItem[] {
...
export default function Results() {
  ...
  useEffect(() => {
    // Reset per-coin state so navigating between coins starts clean.
    setLoading(true);
    setData(null);
    setError(null);
    setAiSentiment(undefined);
    setAiError(null);
    ...
    Promise.all([
      ...
      setData(detail);
      setNews(relevantNews(allNews, detail.name, detail.ticker));
    }).catch(() => {
      setError('Could not load coin data. Make sure you entered a valid coin name or ticker.');
    }).finally(() => setLoading(false));
  }, [coin]);

  // On-demand: only hits the Python/Gemini backend when the user asks for it.
  const analyzeWithAI = async () => {
    ...
    try {
      ...
      if (result.aiSentiment) {
        ...
        setAiError('AI analysis is unavailable right now. Make sure the analysis service is running and try again.');
      }
    } catch (e) {
      logger.warn('app', 'AI sentiment failed on button click', { error: String(e) });
      setAiError('AI analysis failed. Make sure the analysis service is running and try again.');
    } finally {
      setAiLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 md:px-6 py-10 space-y-4">
      ...
      {/* AI Sentiment — on-demand analysis via the Python/Gemini backend */}
      {!aiSentiment && (
        <div className="rounded-xl p-5 md:p-6" style={cardStyle}>
          <div className="flex items-start gap-3 mb-4">
            <Sparkles size={18} style={{ color: '#F7931A', flexShrink: 0, marginTop: 2 }} />
            <div>
              <h2 className="text-strong font-semibold">AI News Analysis</h2>
              <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>
                Analyze the latest {name} headlines with Gemini for a bullish/bearish outlook.
              </p>
            </div>
          </div>
          ...
          {aiError && (
            <p className="text-sm mt-3" style={{ color: COLORS.bearish }}>{aiError}</p>
          )}
        </div>
      )}
      ...
      {aiSentiment && (
        <div
          ...
          style={{ background: 'var(--bg-surface)', border: `1px solid ${aiColor(aiSentiment.classification)}40` }}
        >
          <div className="flex items-center gap-2 mb-4">
            ...
            >
              {aiSentiment.classification} · {aiSentiment.confidence}% confidence
            </span>
          </div>
